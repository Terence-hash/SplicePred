...
最后，为了方便用户使用，使用PyInstaller将Python程序制作成可执行文件SSPT(A Splicing Site Prediction Tool)。SSPT使用方法如下，下载[SSPT.zip](https://github.com/Terence-hash/SplicePred/releases/download/splicing-sites-predictor/SSPT.zip)，进入解压后的文件夹，双击打开SSPT.exe，输入文件(纯序列或FASTA文件格式)或直接输入序列，提交后可自动进行剪接位点的识别，输出结果为识别出的剪接供体位点和剪接受体位点。
